...
class DemPoll {
	...
	function get_vote_screen(){
		...
		$html .= '<form method="POST" id="encuestaForm" action="#democracy-'. $this->id .'">';
			$html .= '<ul class="dem-vote row">';
				...
				foreach( $poll->answers as $answer ){
					...
					$html .= '
					<li class="col px-2" data-aid="'. $answer->aid .'">
			...
			$html .= '<div class="dem-bottom">';
				...
				$btnVoted  = '<div class="dem-voted-button"><input class="dem-button '. democr()->opt('btn_class') .'" type="submit" value="Ya votaste" disabled="disabled"></div>';
				$btnVote   = '<div class="dem-vote-button col-md-4 col-12"><input class="dem-button '. democr()->opt('btn_class') .'" type="submit" value="Votar" data-dem-act="vote"></div>';
	...
	function get_result_screen(){
		...
		$html .= '<ul class="dem-answers" data-voted-class="'. $voted_class .'" data-voted-txt="'. $voted_txt .'">';
			...
			foreach( $answers as $answer ){
				...
				$percent_txt = sprintf( _x('%s - %s%% de todos los votos','front','democracy-poll'), $__sclonenie( $votes, _x('voto,votos,votos','front','democracy-poll') ), $percent );
				$title       = ( $is_voted_this ? $voted_txt : '' ) . ' '. $percent_txt;
				$title       = " title='$title'";

				$votes_txt = $votes .' '. '<span class="votxt">'. $__sclonenie( $votes, _x('voto,votos,votos','front','democracy-poll'), 'nonum' ) .'</span>';
		...
		$html .= '<div class="dem-bottom">';
			$html .= '<div class="dem-poll-info">';
				$html .= '<div class="dem-total-votes">'. sprintf( 'Total de votos %s', $total ) .'</div>';
