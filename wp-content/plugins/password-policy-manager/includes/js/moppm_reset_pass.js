				...
				var nonce = jQuery("input[name ='NONCE']").val(); ;	
					 var data = {
						...
						jQuery.post(ajax_object.ajaxurl, data, function(response) 
						{
                            ...
                            { 
                            	Moppm_error_msg("Your current password is wrong");
                            }
                            else if(response =='PASS_NOT_MATCH')
                            {
                            	Moppm_error_msg("Your Passwords are mismatched");
                            }else if(response=='SPACE_FOUND'){
                                Moppm_error_msg("Passwords cannot have a blank space.");
                            }
                            else if(response == 'ERROR'){
                                Moppm_error_msg("An unknown error occured. please try again later!");
                            }
                            else if(response =='SESSION_TIMEOUT')
                            {  
                                Moppm_error_msg('Session Timeout',true); 
                                document.location.href = ajax_object.loginUrl; 
                            }else if(response =='Login')
                            {  
                                Moppm_success_msg('Your Password is successfully changed',true); 
...
function Moppm_error_msg(error) 
{
	...
	var msg = "<div id='notice_div' class='moppm_reset_msg_error'><div class='popup_text'>&nbsp; &nbsp; "+error+"</div></div>";
...
{
jQuery('#moppm_message').empty();
var msg = "<div id='notice_div' class='moppm_reset_msg_success'><div class='popup_text'>&nbsp; &nbsp; "+success+"</div></div>";
jQuery('#moppm_message').append(msg);
window.onload = Moppm_nav_popup();
}
