...
class MOPPMFeedbackHandler
{
    ...
    function handle_feedback($postdata)
    {
        ...
        $deactivation_reason = isset($_POST['moppm_feedback'])? sanitize_text_field($_POST['moppm_feedback']):'NA';

        if($deactivation_reason=='other' || $deactivation_reason == 'specific_feature')
            $deactivate_reason_message =  '['.$deactivation_reason.']-'.sanitize_text_field($_POST['moppm_query_feedback']) ;
        elseif($deactivation_reason=='It_is_not_what_I_am_looking_for'){
            $deactivate_reason_message =  '[It is not what I am looking for] -'.sanitize_text_field($_POST['moppm_query_feedback_specific_feature']) ;
        }
        else
        ...
        $reply_required = '';
        if (isset($_POST['moppm_get_reply'])) {
            $reply_required = htmlspecialchars(sanitize_text_field($_POST['moppm_get_reply']));
        }
        ...
        $moppm_send_conf = '';
        if(isset($_POST['moppm_send_conf'])){
            $moppm_send_conf = htmlspecialchars(sanitize_text_field($_POST['moppm_send_conf']));
        }
        error_log("hello 2");
