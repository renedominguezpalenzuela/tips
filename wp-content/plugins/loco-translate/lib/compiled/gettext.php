...
public function offsetExists($k ){ return ! is_null( $this->normalize($k) ); } 
#[ReturnTypeWillChange]
public function offsetGet($k ){ $k = $this->normalize($k); if( is_null($k) ){ return ''; } return parent::offsetGet($k); } 
#[ReturnTypeWillChange]
public function offsetSet($key, $v ){ $k = strtolower($key); if( isset($this->map[$k]) && $key !== $this->map[$k] ){ parent::offsetUnset( $this->map[$k] ); } $this->map[$k] = $key; parent::offsetSet( $key, $v ); } 
#[ReturnTypeWillChange]
...
protected function setHeader( LocoPoHeaders $head ){ $this->head = $head; $cs = $head->getCharset(); if( is_string($cs) && '' !== $cs ){ if( '' === $this->cs ){ $this->setCharset($cs); } } return $head; } 
...
public function fsniff( $str ){ return loco_sniff_js_printf($str) ? 'javascript' : ''; } 
